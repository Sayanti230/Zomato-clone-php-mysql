...
<div class="container">
	<h1 class="mt-5">Cart <button class="btn btn-danger btn-lg"><i class="fas fa-shopping-cart"></i></button></h1>
		...
		<div class="col-8">
			...
			while($row = mysqli_fetch_assoc($result)){
				...
				echo '<div class="card mt-3" id="product-card'.$row['product_id'].'">
					<div class="row">
						<div class="col-3">
							<img src="'.$row['bg'].'" style="width: 100%;height: 130px;margin-top: 10px;">
						</div>
						<div class="col-7">
							<h3 class="mt-3">'.$row['name'].'</h5>
							<h5 class="mt-1 text-muted">'.$row['details'].'</h5>
							<h5 class="mt-1 text-muted">'.$row['delivery'].'</h5>
							<h5 class="text-muted">Rs <span id="price'.$row['product_id'].'">'.$row['price'].'</span></h5>

						</div>
						<div class="col-2">
							<div class="mt-4">
								<button data-id='.$row['product_id'].' class="btn btn-danger btn-sm text-white plus-one">-</button>
			...
			if($counter == 0){
				...
				echo '<hr>
					  ...
					  <a href="place_order.php" class="btn btn-danger btn-lg text-white" style="float: right">Checkout</a>';
			}
